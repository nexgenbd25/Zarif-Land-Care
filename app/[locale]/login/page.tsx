...
function LoginForm() {
  ...
  const content = {
    ...
    mfaRequired_bn: '৬ ডিজিটের কোড দিন',
    mfaRequired_en: 'Enter all 6 digits',
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (factorsResult.success && factorsResult.totp) {
        const verifiedFactors = factorsResult.totp.filter(
          (f: any) => f.status === 'verified'
        );

        if (verifiedFactors.length > 0) {
          // 2FA enabled → show MFA challenge
  ...
  const handleMfaVerify = async (e: React.FormEvent) => {
    ...
    if (mfaCode.length !== 6) {
      setToast({ type: 'error', message: t('mfaRequired') });
      return;
    }

    if (!mfaFactorId) {
      setToast({ type: 'error', message: t('mfaInvalid') });
      return;
    }
    ...
    try {
      const result = await challengeAndVerify(mfaFactorId, mfaCode);
      ...
      // ✅ MFA success
      setToast({ type: 'success', message: t('loginSuccess') });
      redirectBasedOnRole(pendingRole || 'user');
    } catch (err) {
      console.error('MFA exception:', err);
      setToast({ type: 'error', message: t('mfaInvalid') });
      setIsLoading(false);
    }
  };
    ...
    return (
      <section className="relative min-h-[100dvh] w-full flex items-center justify-center px-4 py-3 sm:py-6 bg-gradient-to-br from-[#F0FDF4] via-white to-[#F0FDF4] overflow-hidden">
        ...
        >
          ...
          <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-lg p-6 sm:p-8">
            {/* Shield Icon */}
            ...
            <div className="text-center mb-6">
              <h2 className="text-xl sm:text-2xl font-bold text-[#1F2937] text-bangla-heading mb-2">
                {t('mfaTitle')}
              </h2>
              <p className="text-xs sm:text-sm text-[#6B7280] text-bangla-safe leading-relaxed">
                {t('mfaSub')}
              </p>
            </div>
            ...
            <form onSubmit={handleMfaVerify} className="space-y-4">
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-[#1F2937] mb-2 text-bangla-safe text-center">
                  {t('mfaCodeLabel')}
                </label>
                <input
                  type="text"
                  ...
                  disabled={isLoading}
                  autoFocus
                  className="w-full px-4 py-4 rounded-lg border-2 border-[#E5E7EB] bg-white text-center text-2xl font-bold tracking-[0.5em] text-[#1F7A3F] placeholder-[#9CA3AF] placeholder:tracking-normal focus:outline-none focus:ring-2 focus:border-[#1F7A3F] focus:ring-[#1F7A3F]/20 disabled:opacity-60"
                />
              </div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-6 py-3 rounded-lg font-bold text-white text-sm bg-[#1F7A3F] hover:bg-[#155E30] shadow-md shadow-[#1F7A3F]/20 transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed active:scale-[0.98]"
              >
                {isLoading ? (
                  <>
                  ...
                  <>
                    <Shield size={16} />
                    <span className="text-bangla-safe">{t('mfaVerify')}</span>
                    <ArrowRight size={16} />
                  </>
                )}
              </button>
              ...
              <button
                ...
                disabled={isLoading}
                className="w-full px-6 py-2.5 rounded-lg font-semibold text-sm text-[#4B5563] bg-[#F8FAF9] border border-[#E5E7EB] hover:bg-white transition-all disabled:opacity-60 text-bangla-safe"
              >
                {t('mfaBack')}
              </button>
            </form>
